...
- KYRO read-only API via `/api/runs`, `/api/run-detail`, `/api/friends-runs`
...
로컬 개발 서버는 `/api/runs`, `/api/run-detail`, `/api/friends-runs` 요청을 실제 KYRO API의 `/api/v1/*`로 프록시하고, 서버 측에서 `Authorization: Bearer <KYRO_API_KEY>`를 붙입니다.
...
Vercel 배포에서는 `api/runs.js`, `api/run-detail.js`, `api/friends-runs.js`가 같은 역할을 수행합니다.
